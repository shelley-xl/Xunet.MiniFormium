...
/// <summary>
/// MiniFormiumApplicationBuilder扩展
/// </summary>
public static class MiniFormiumApplicationBuilderExtensions
{
    /// <summary>
    /// 添加MiniFormium
    /// </summary>
    ...
    /// <returns></returns>
    public static IServiceCollection AddMiniFormium<T>(this IServiceCollection services) where T : Form
    {
        services.AddSingleton(new Mutex(false, typeof(T).Assembly.GetName().Name));

        services.AddSingleton<PropertyManager>();

        var MiniFormiumOpts = new MiniFormiumOptions(services);

        var createAction = MiniFormiumOpts.UseMiniFormium<T>();

        if (createAction != null)
        {
            services.AddSingleton(MiniFormiumOpts);
    ...
    /// <returns></returns>
    public static IServiceCollection AddMiniFormium<T>(this IServiceCollection services, Action<StartupOptions> setupOptions) where T : Form, IMiniFormium
    {
        ...
        if (startupOptions != null && startupOptions.Storage != null)
        {
            ...
            {
                var assemblyName = Assembly.GetExecutingAssembly().GetName().Name ?? "Xunet.MiniFormium";
    ...
    public static IServiceCollection AddWebApi(this IServiceCollection services, Action<ServiceProvider, IServiceCollection> setupAction)
    {
        ...
        builder.Services.AddSwaggerGen(x =>
        {
            ...
            var xmlPath2 = Path.Combine(AppContext.BaseDirectory, $"{Assembly.GetEntryAssembly()?.GetName().Name}.xml");
        ...
        app.Use(async (context, next) =>
        {
            try
            {
                ...
                {
                    throw new MiniFormiumNotFoundException();
                }
            }
            catch (MiniFormiumNotFoundException ex)
            {
            ...
            catch (MiniFormiumFailureException ex)
            {
                await context.Response.WriteAsJsonAsync(new OperateResultDto
                {
            ...
            catch (MiniFormiumInvalidParameterException ex)
            {
                await context.Response.WriteAsJsonAsync(new OperateResultDto
                {
